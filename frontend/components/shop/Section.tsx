    ...
    <section ref={ref} className={`mx-auto mb-12 max-w-7xl px-4 sm:px-6 lg:px-8 ${isVisible ? "animate-fadeInUp" : "opacity-0"} ${className}`}>
      {title || subtitle || action ? (
        <div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
          <div>
            {title ? <h2 className="text-xl font-medium text-white sm:text-2xl">{title}</h2> : null}
            {subtitle ? <p className="mt-1 text-sm text-[#A0A0A0]">{subtitle}</p> : null}
          </div>
          {action ? (
            <Link href={action.href} className="inline-flex items-center gap-2 text-sm text-[#A0A0A0] hover:text-white">
              {action.label}
