...
export function FlashSalePreview({ sales }: Readonly<{ sales: FlashSale[] }>) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border-default bg-[linear-gradient(135deg,#141414,#080808)]">
      <div className="grid gap-6 p-5 lg:grid-cols-[280px_1fr] lg:p-6">
        <div className="flex flex-col justify-between gap-5">
          <div>
            <p className="inline-flex items-center gap-2 text-sm font-medium uppercase text-status-warning">
        ...
        <div className="-mx-5 overflow-x-auto px-5 [scrollbar-width:none] lg:mx-0 lg:px-0 [&::-webkit-scrollbar]:hidden">
          <div className="flex gap-4">
            {sales.slice(0, 4).map((sale) => {
              ...
              return (
                <Link className="w-56 shrink-0 rounded-lg border border-border-default bg-bg-primary p-4 transition hover:border-border-hover" href={`/flash-sale/${sale.id}`} key={sale.id}>
                  ...
                  <p className="mt-1 line-clamp-1 text-xs text-text-secondary">{sale.productName} · {sale.specName}</p>
                  <div className="mt-3 flex items-end gap-2">
                    <span className="text-lg font-semibold text-status-danger">NT$ {sale.flashPrice.toLocaleString()}</span>
                    {sale.originalPrice ? <span className="pb-0.5 text-xs text-text-tertiary line-through">NT$ {sale.originalPrice.toLocaleString()}</span> : null}
                  </div>
                  <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/10">
                    <div className="h-full rounded-full bg-white" style={{ width: `${percent}%` }} />
                  </div>
                  <p className="mt-2 text-xs text-text-secondary">{remaining} / {sale.totalStock} left</p>
                </Link>
              );
            })}
          </div>
