...
export function ContentCard({ content, featured = false }: Readonly<ContentCardProps>) {
  ...
  return (
    <Link
      ...
      className={`group block overflow-hidden rounded-xl border border-[#2A2A2A] bg-[#141414] transition hover:-translate-y-0.5 hover:border-[#404040] ${
        featured ? "md:grid md:grid-cols-[1.25fr_0.75fr]" : ""
      }`}
    >
      <div className={`relative bg-[#1E1E1E] ${featured ? "aspect-video md:aspect-auto" : "aspect-[4/3]"}`}>
      ...
      <div className={featured ? "p-6 md:p-8" : "p-4"}>
        ...
        <h2 className={`mt-4 line-clamp-2 font-semibold text-white ${featured ? "text-2xl md:text-3xl" : "text-lg"}`}>{content.title}</h2>
        {content.summary || content.subtitle ? (
          <p className="mt-3 line-clamp-2 text-sm leading-6 text-[#A0A0A0]">{content.summary ?? content.subtitle}</p>
        ) : null}
