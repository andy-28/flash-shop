...
export default function AdminDashboardPage() {
  ...
  const recentColumns: Column<RecentOrder>[] = [
    { key: "orderNo", header: "Order", width: "1fr", render: (row) => <div><p className="font-medium">{row.orderNo}</p><p className="text-xs text-text-tertiary">{new Date(row.createdAt).toLocaleString()}</p></div> },
    { key: "userName", header: "User", width: "1fr", render: (row) => <div><p>{row.userName}</p><p className="text-xs text-text-tertiary">{row.userEmail}</p></div> },
  ...
  if (!data) return <p className="text-sm text-status-danger">Unable to load dashboard.</p>;
  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <span className="inline-flex items-center gap-2 rounded-md border border-border-default bg-bg-secondary px-3 py-2 text-sm text-text-secondary"><span className={`size-2 rounded-full ${isConnected ? "bg-status-success" : "bg-status-danger"}`} />{isConnected ? "Live updates on" : "Connecting..."}</span>
        <span className="rounded-md border border-border-default bg-bg-secondary px-3 py-2 text-sm text-text-secondary">Total revenue {formatCurrency(data.totalRevenue)}</span>
      </div>
      {inventoryAlert ? <div className="mb-4 flex items-center gap-2 rounded-md border border-status-warning/30 bg-status-warning/10 px-4 py-3 text-sm text-status-warning"><Radio className="size-4" />{inventoryAlert}</div> : null}
      ...
      <section className="mb-6 grid gap-4 xl:grid-cols-2">
        <Chart title="Order trend"><LineChart data={data.dailyStats}><CartesianGrid stroke="var(--border-default)" vertical={false} /><XAxis dataKey="date" tickFormatter={(value) => String(value).slice(5)} stroke="var(--text-secondary)" tickLine={false} /><YAxis stroke="var(--text-secondary)" tickLine={false} allowDecimals={false} /><Tooltip contentStyle={{ background: "var(--bg-tertiary)", border: "1px solid var(--border-default)", color: "var(--text-primary)" }} /><Line dataKey="orderCount" name="Orders" stroke="var(--accent-primary)" strokeWidth={2} dot={{ r: 3 }} /></LineChart></Chart>
        <Chart title="Revenue trend"><BarChart data={data.dailyStats}><CartesianGrid stroke="var(--border-default)" vertical={false} /><XAxis dataKey="date" tickFormatter={(value) => String(value).slice(5)} stroke="var(--text-secondary)" tickLine={false} /><YAxis stroke="var(--text-secondary)" tickFormatter={(value) => `${Number(value) / 1000}k`} tickLine={false} /><Tooltip contentStyle={{ background: "var(--bg-tertiary)", border: "1px solid var(--border-default)", color: "var(--text-primary)" }} formatter={(value) => formatCurrency(Number(value))} /><Bar dataKey="revenue" name="Revenue" fill="var(--status-success)" radius={[4, 4, 0, 0]} /></BarChart></Chart>
      </section>
      <section className="mb-6 grid gap-4 xl:grid-cols-2">
...
function Chart({ children, title }: Readonly<{ children: ReactElement; title: string }>) {
  return <div className="rounded-md border border-border-default bg-bg-secondary p-4"><div className="mb-4 flex items-center justify-between"><h2 className="font-semibold">{title}</h2><span className="text-xs text-text-tertiary">Last 7 days</span></div><div className="h-72"><ResponsiveContainer width="100%" height="100%">{children}</ResponsiveContainer></div></div>;
}
