  ...
  return (
    <div className="min-h-screen bg-bg-primary text-text-primary">
      ...
      <main className="mx-auto max-w-4xl px-4 py-8">
        <p className="text-xs uppercase tracking-[0.22em] text-text-secondary">Notifications</p>
        <h1 className="mt-3 text-4xl font-semibold">通知中心</h1>

        {isLoading ? <div className="mt-8 h-64 shimmer rounded-xl" /> : null}
        {!isLoading && notifications.length === 0 ? <p className="mt-8 rounded-md border border-border-default bg-bg-secondary p-6 text-text-secondary">目前沒有通知。</p> : null}
        <div className="mt-8 grid gap-3">
          {notifications.map((notification) => (
            <button
              key={notification.id}
              type="button"
              className={`rounded-md border border-border-default bg-bg-secondary p-4 text-left hover:border-border-hover ${notification.isRead ? "" : "border-l-4 border-l-[#8B5CF6]"}`}
            ...
            >
              ...
              <p className="mt-3 text-xs text-text-tertiary">{new Date(notification.createdAt).toLocaleString()}</p>
            </button>
          ))}
        </div>
      </main>
